...
[![Release](https://img.shields.io/github/v/release/Chronixx4/minedocker)](https://github.com/Chronixx4/minedocker/releases)

**minedocker** ist ein selbst gehostetes Web-Dashboard zur Verwaltung beliebig
vieler Minecraft-Server auf einem einzigen Docker-Host — zum Beispiel auf einem
NAS, einem Home-Server oder einem VPS.

Statt sich per Hand durch Container-Konfiguration, Ports und `docker run`-Befehle
zu arbeiten, erledigt das Dashboard alles über die Weboberfläche: Server
erstellen, starten, stoppen, Mods und Modpacks installieren, Welten sichern,
Spieler verwalten, Logs mitverfolgen. FastAPI-Backend + Vanilla-JS-Frontend —
ohne Framework-Abhängigkeiten im Browser und ohne dass man Java- oder
Docker-Kenntnisse für den täglichen Betrieb braucht.

Es gibt **keinen festen „Hauptserver"**: Jeder Minecraft-Server ist eine Instanz
mit eigenem Ordner, eigenem Port, eigenem RAM-Limit und eigenem
`itzg/minecraft-server`-Container — erstellt, gestartet und überwacht über das
Dashboard.
...
- **Übersicht mit Live-Daten** — CPU/RAM (Sparklines), Spieler online
  (Server-List-Ping), Uptime, MOTD, Statusfilter, Suche, Sortierung, Gruppen-
  und Sammel-Aktionen
...
- **Datapacks** — hochladen (nur gestoppt), aktivieren/deaktivieren (bei
  laufendem Server per RCON), löschen; deaktivierte Packs liegen in
  `disabled_datapacks/`
- **Datei-Browser** — Instanz-Ordner durchstöbern, Textdateien ≤ 1 MiB
  bearbeiten, Dateien hochladen/umbenennen/löschen; verwaltete Dateien
  (`server.properties`, `instance.json`, `packs/`) sind geschützt
- **Spielzeit je Spieler** — Leaderboard 24 h/7 Tage/30 Tage/Gesamt aus
  Session-Tracking des Verlauf-Samplers (SLP-Spielerliste, RCON-Fallback)
- **Login & Rollen** — Benutzer mit Admin/Viewer-Rolle, sicheres Session-Cookie,
  Setup-Dialog für den ersten Admin, Sperrung nach 10 Fehlversuchen;
  Passwort-Änderung und Rollen-Downgrade invalidieren ausgestellte Sessions
  sofort; `DASHBOARD_API_KEY` bleibt als Admin-Bypass für Skripte
- **Zeitplan je Instanz** — Auto-Start, täglicher Neustart und täglicher Stopp
  (jeweils mit Vorwarnung), geplante Backups mit Rotation, geplanter
  Mod-Update-Check (Container-Lokalzeit)
...
- **Statistik-Verlauf** — persistente CPU/RAM-/Spieler-Charts über 6 h bis
  30 Tage
- **Live-Logs per SSE** — Log-Fenster aktualisiert sich in Echtzeit, auch bei
  gestoppten Instanzen (automatischer Polling-Fallback)
- **Einstellungs-Editor** für `server.properties` (Formular- oder Raw-Modus,
  verwaltete Keys geschützt)
- **Sicherheit zuerst** — vor jeder Modpack-Installation/Restore wird
  automatisch ein Wiederherstellungs-Snapshot angelegt (fail-closed)
...
Danach ist das Dashboard unter `http://<host-ip>:8080` erreichbar — im Tab
„Server" den ersten Server erstellen, fertig. Die Instanz-Container werden vom
Dashboard selbst erzeugt (der nötige Docker-Socket-Mount ist in der
Compose-Datei enthalten).
...
Statt selbst zu bauen kann auch das fertige Image genutzt werden:
`ghcr.io/chronixx4/minedocker:latest` (amd64 + arm64, gebaut bei jedem Release).

## ZimaOS / NAS-App-Import

Für die Installation über die ZimaOS-UI gibt es eine angepasste Import-YAML
ohne Build-Schritt und ohne `$`-Escapes (beides Eigenheiten des
ZimaOS-Import-Dialogs):
...
Beim ersten Aufruf öffnet das Dashboard den **Setup-Dialog** — der erste
angelegte Benutzer wird automatisch Admin. Danach schützt das Login alle
`/api`-Routen:
...
- **Ohne Setup** (kein Benutzer, kein `DASHBOARD_API_KEY`): API offen —
  bewusstes Homelab-Verhalten für geschlossene Netze
...
- Das Dashboard hat vollen Zugriff auf die Docker-Engine (Socket-Mount) und
  verwaltet Server-Container — **nur im LAN/VPN betreiben** (WireGuard/
  Tailscale) oder hinter einen authentifizierten Reverse-Proxy hängen.
- Login & Rollen nutzen (Setup-Dialog beim ersten Aufruf) und/oder
  `DASHBOARD_API_KEY` setzen, sobald das Netzwerk nicht vollständig
  vertrauenswürdig ist. Bei Reverse-Proxy: HTTPS-Terminierung am Proxy
  (Cookie ist HttpOnly + SameSite=Strict; `secure`-Flag ist bewusst nicht
  gesetzt, damit HTTP-LAN-Zugänge funktionieren).
- Das Dashboard läuft als unprivilegierter User (UID/GID 1000),
  Docker-Socket-Zugriff nur über die Socket-Gruppe (`DOCKER_GID`).
...
(Interne Projektlandkarte: Struktur, Endpunkte, Verhalten, bekannte
Eigenheiten).
